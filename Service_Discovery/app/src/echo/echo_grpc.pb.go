...
const (
	...
	EchoServer_SayHelloBidStream_FullMethodName = "/echo.EchoServer/SayHelloBidStream"
)
...
type EchoServerClient interface {
	...
	SayHelloBidStream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EchoRequest, EchoReply], error)
}
...
func (c *echoServerClient) SayHelloBidStream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EchoRequest, EchoReply], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &EchoServer_ServiceDesc.Streams[1], EchoServer_SayHelloBidStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[EchoRequest, EchoReply]{ClientStream: stream}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EchoServer_SayHelloBidStreamClient = grpc.BidiStreamingClient[EchoRequest, EchoReply]
...
type EchoServerServer interface {
	...
	SayHelloBidStream(grpc.BidiStreamingServer[EchoRequest, EchoReply]) error
...
func (UnimplementedEchoServerServer) SayHelloBidStream(grpc.BidiStreamingServer[EchoRequest, EchoReply]) error {
	return status.Errorf(codes.Unimplemented, "method SayHelloBidStream not implemented")
}
...
func _EchoServer_SayHelloBidStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(EchoServerServer).SayHelloBidStream(&grpc.GenericServerStream[EchoRequest, EchoReply]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EchoServer_SayHelloBidStreamServer = grpc.BidiStreamingServer[EchoRequest, EchoReply]
...
var EchoServer_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SayHelloBidStream",
			Handler:       _EchoServer_SayHelloBidStream_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
